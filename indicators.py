    ...
    if is_flat:
      if c <= bb_lower or (abs(c - l_sup) / c < 0.005 and rsi < 38):
        return {
            ...
            "reason": (
                f"Флєт (ADX: {round(adx, 1)}) + Відскок від нижньої межі BB /"
                f" підтримки (RSI: {round(rsi, 1)})"
            ),
            "global_trend": global_trend,
            "mid_trend": mid_trend,
        ...
        return {
            ...
            "adx": round(float(adx), 2),
            "reason": (
                f"Флєт (ADX: {round(adx, 1)}) + Відскок від верхньої межі BB /"
                f" опору (RSI: {round(rsi, 1)})"
            ),
            "global_trend": global_trend,
            "mid_trend": mid_trend,
            "local_trend": local_trend,

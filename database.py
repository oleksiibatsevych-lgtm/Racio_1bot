...
def init_db():
    try:
        ...
        # Таблиця для логів відхилень (замість SQLite)
        cursor.execute("""
...
def update_signal_result(signal_id, result, exit_price, pips=0, status='CLOSED'):
    try:
        ...
        logger.error(f"Помилка оновлення сигналу #{signal_id}: {e}")
...
# --- НОВІ ФУНКЦІЇ ДЛЯ ЛОГУВАННЯ ВІДХИЛЕНЬ ---
def save_filtered_log(chat_id, log_text):
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO filtered_logs (chat_id, log_text, timestamp) VALUES (%s, %s, %s)", 
...
def get_system_logs(chat_id, minutes=60):
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cutoff = time.time() - (minutes * 60)
        cursor.execute("SELECT log_text FROM filtered_logs WHERE chat_id = %s AND timestamp > %s ORDER BY timestamp DESC LIMIT 20", (chat_id, cutoff))
